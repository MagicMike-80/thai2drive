...
async def stripe_webhook(request: Request):
    ...
    try:
        if event_type == "checkout.session.completed":
            ...
            if subscription_id:
                ...
                # Direct / one-time invoice (no subscription) — find user by customer email
                ...
                if customer_email:
                    user = await db.users.find_one({"email": customer_email.strip().lower()}, {"_id": 0})
                    if user:
                        await _set_user_premium(
                            user["id"],
                            plan_id="monthly",
                        ...
                        logger.info("Premium granted via direct invoice for %s", customer_email)
                    else:
                        logger.warning("Invoice paid but no user found for email %s", customer_email)

    except Exception as exc:
        logger.warning("Stripe webhook handling failed for %s: %s", event_type, exc)
        await db.stripe_events.update_one(
            {"event_id": event.get("id")},
            {"$set": {"event_id": event.get("id"), "type": event_type, "livemode": is_live, "processing_error": str(exc), "processed_at": datetime.now(timezone.utc).isoformat()}},
            upsert=True,
        )
    ...
    await db.stripe_events.update_one(
        {"event_id": event.get("id")},
        {"$set": {"event_id": event.get("id"), "type": event_type, "livemode": is_live, "processed_at": datetime.now(timezone.utc).isoformat()}},
        upsert=True,
    )
...
async def admin_grant_premium(
    data: PremiumGrantRequest,
    _: dict = Depends(require_admin),
):
    """
    Admin: manually grant or revoke premium for a user by email.
    Use when a payment was confirmed outside the normal checkout flow
    (e.g. test payment, bank transfer, promo).
    """
    email = data.email.strip().lower()
    now = datetime.now(timezone.utc).isoformat()
    update: Dict[str, Any] = {
        ...
        "premium_plan": data.plan_id,
        "premium_status": "active" if data.is_premium else "canceled",
        "premium_updated_at": now,
    }
    if data.note:
        update["premium_note"] = data.note
    result = await db.users.update_one({"email": email}, {"$set": update})
    if result.matched_count == 0:
        raise HTTPException(status_code=404, detail=f"User not found: {email}")
    action = "GRANTED" if data.is_premium else "REVOKED"
    logger.info("Admin: premium %s for %s (plan: %s)", action, email, data.plan_id)
    return {
        "ok": True,
        "email": email,
        "is_premium": data.is_premium,
